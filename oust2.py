...
class OustGame(psmoveapi.PSMoveAPI):
    ...
    def handle_lobby_update(self, controller):
        # Track poll failures
        # (In the new API, we don't need to manually poll, but we can still track disconnects)
        ...
        if controller.serial in self.controllers_alive:
            ...
            controller.color = psmoveapi.RGB(0.1, 0.05, 0.0)
        ...
        # SELECT disconnects the move (button value 256 = SELECT)
        if controller.buttons & (1 << 8):  # SELECT button
    ...
    def sleep_controllers(self, sleep_time, leds, rumble):
        ...
        while time.time() < pause_time:
            for serial, controller in self.controllers_alive.items():
                controller.color = psmoveapi.RGB(leds[0]/255.0, leds[1]/255.0, leds[2]/255.0)
    ...
    def start_game(self):
        ...
        # Countdown sequence
        # White
        self.sleep_controllers(0.5, (255, 255, 255), 0)
        # White/rumble
        self.sleep_controllers(0.3, (255, 255, 255), 100)
        # Red/no rumble
        self.sleep_controllers(0.75, (50, 0, 0), 0)
        # Yellow
        self.sleep_controllers(0.75, (50, 75, 0), 0)
        # Green
        self.sleep_controllers(0.75, (0, 50, 0), 0)
...
if __name__ == '__main__':
    print("=" * 60)
    print("OUST2 - PS Move Game")
    print("=" * 60)
    print("\nPairing workflow:")
    print("1. Plug in PS Move controller via USB")
    print("2. Run 'sudo ./psmove pair' in another terminal")
    print("3. Wait for pairing to complete, then unplug USB")
    print("4. Press the PS button on the controller")
    print("5. Controller should connect via Bluetooth")
    print("\nStarting game...\n")
